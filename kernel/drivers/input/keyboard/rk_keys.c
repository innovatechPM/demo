...
extern volatile  int kdxf_stats;
...
static irqreturn_t kdxf_isr(int irq, void *dev_id)
{
	struct rk_keys_button *button = (struct rk_keys_button *)dev_id;
	struct rk_keys_drvdata *pdata = dev_get_drvdata(button->dev);
	struct input_dev *input = pdata->input;
	...
	if (!pdata->in_suspend) {
		printk("@@wyj in_suspend = %d\n",pdata->in_suspend);
		input_event(input, EV_KEY, KEY_F1, 1);
		input_sync(input);
		input_event(input, EV_KEY, KEY_F1, 0);
		input_sync(input);
	}else{
		printk("@@wyj in_suspend = %d\n",pdata->in_suspend);
			    ...
			    struct platform_device *pdev)
{
	...
	for_each_child_of_node(node, child_node) {
		...
		printk("@@ %s,%d,%d,%d,%s\n",__FUNCTION__,__LINE__,pdata->button[i].code,pdata->button[i].type,pdata->button[i].desc);
		...
		case TYPE_GPIO:
			...
			printk("@@ %s,%d,%d,%d\n",__FUNCTION__,__LINE__,pdata->button[i].gpio,flags);
...
static int keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ddata->nbuttons; i++) {
		...
		if (button->type == TYPE_GPIO) {
			...
			if(!strcmp(button->desc, "kdxf_eint")){
				printk("@@ %s,%d\n",__FUNCTION__,__LINE__);
				error = devm_request_irq(dev, irq, kdxf_isr,
							 button->active_low ?
